...
import { CropAnalysis, WeatherInfo, Recommendation } from '../services/api';
...
interface Feature {
  ...
  stats: string;
  requiredRole: 'admin' | 'farmer' | 'viewer';
}
...
const Dashboard: React.FC = () => {
  ...
  const [lightMode, setLightMode] = useState<'light' | 'dark' | 'auto'>('auto');
  ...
  const fetchWeatherData = async () => {
    setLoading(true);
    try {
      const weatherInfo = await api.getWeatherInfo();
      setWeatherData({
        temperature: weatherInfo.temperature,
        humidity: weatherInfo.humidity,
        description: weatherInfo.forecast,
        windSpeed: 0 // Add this to your WeatherInfo interface if needed
      });
      setError(null);
    } catch (err) {
    ...
    try {
      ...
      setTimeout(async () => {
        if (videoRef.current) {
          ...
          await analyzeQuickScan(imageData);
  ...
  const analyzeQuickScan = async (imageData: string) => {
    try {
      setLoading(true);
      ...
      speakAlert(`Crop analysis complete. Health status: ${mockResult.health}`);
      setLoading(false);
    } catch (err) {
            ...
            >
              ...
              <CardActions>
                <Button 
                  ...
                  onClick={() => {
                    setError(null);
                    if (feature.path === '/crop-analysis' && !hasPermission('farmer')) {
                      setError('You need farmer privileges to access Crop Analysis');
                      return;
                    }
